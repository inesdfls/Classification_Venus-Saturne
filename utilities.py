...
def load_images(dataset_dir, pattern="*.jpg"):
    """Load images from directory into a pandas Series with filename as index."""
    image_paths = sorted(glob(os.path.join(dataset_dir, pattern)))
    images = pd.Series(
        [Image.open(img_path) for img_path in image_paths],
        index=[os.path.basename(img_path) for img_path in image_paths]
    )
    return images

def image_grid(images, titles=None, ncols=5, figsize=(10, 5), cmap=None, title_fontsize=10):
    """Display a grid of images with no spacing between rows.
    ...
    Args:
        ...
        cmap: Color map for single-channel images
    ...
    for i, (ax, img) in enumerate(zip(axes.flat, images)):
        ...
        if titles is not None and i < len(titles):
            ax.set_title(titles[i], fontsize=title_fontsize, y=1.0)
